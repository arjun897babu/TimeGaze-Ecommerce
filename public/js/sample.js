...
let previewDiv = document.querySelector('.imageSecondPreview-div');
let imageelement = document.querySelectorAll('#productName');

const imageElement = Array.from(imageelement).map((element) => {
...
function checkInputValidity(form, element) {
  ...
  if (imageElement.length + fileInput.files.length < 1) {
    isValid = false;
...
function setupImagePreview() {
  ...
  if (imageElement.length + files.length + currentList.length <= 4) {
...
function imageValidationCheck() {
  ...
  let validate = true;
  if (imageElement.length + currentList.length < 1) {
    validate = false
    displayErrorMessage(fileInput, erroMessage.fileInput.invalid);
    fileInput.classList.add('is-invalid');
    fileInput.classList.remove('is-valid');
